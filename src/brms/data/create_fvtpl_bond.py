...
Bond: issued 2020-06-01, 5Y, 4% semi-annual coupon, matures 2025-06-01.
Deposit carries 0% interest.  Bond acquisition cost is QuantLib NPV at
acquisition date.  The bond is classified as Fair Value Through Profit
or Loss (FVTPL) in the trading book — mark-to-market gains and losses
...
_OUT_PATH = _DATA_DIR / "fvtpl_bond.zip"
_START_DATE = "2022-01-04"

# ── Instrument & position definitions ────────────────────────────────────

_ISSUER_CORP = {"name": "Acme Corp", "issuer_type": "CORPORATE", "credit_rating": "BBB"}
...
INSTRUMENTS = [
    ...
    {"id": "deposit-001", "type": "deposit", "name": "Customer Deposit", "interest_rate": 0.0},
    {
        ...
        "name": "5Y Corporate Bond 4% (FVTPL)",
...
def create_fvtpl_zip(data_folder: Path, out_path: Path | None = None) -> Path:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:  # noqa: PLR2004
        print("Usage: python create_fvtpl_bond.py <data_folder>", file=sys.stderr)  # noqa: T201
        sys.exit(1)
    path = create_fvtpl_zip(data_folder=Path(sys.argv[1]))
